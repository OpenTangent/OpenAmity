...
class GeminiWorker:
    def __init__(self, agent_id=None):
        ...
        logging.debug(
            f"GeminiWorker.__init__ called. Existing thought_received: {hasattr(self, 'thought_received')}")
        self.client = None
        self.session = None
        # text, list of function calls
        ...
        if not self.api_key:
            ...
            if self.settings.get("core.first-run", False):
                logging.info(
                    "GEMINI_API_KEY not found in .env (expected on first run)")
            else:
                logging.error(self.last_error)
                self.error_occurred.emit(
                    "Missing API Key. Please check .env file.")
        else:
            try:
    ...
    def start_session(self, system_instruction=None, tools=None):
        ...
        if not getattr(self, 'available', False):
            logging.error(
                "Attempted to start session but GeminiWorker is unavailable.")
            self.error_occurred.emit(
                "Session Start Error: Gemini Worker is not available")
            return

        logging.debug(
            "Gemini client successfully initialized for start_session")
        ...
        try:
            ...
            logging.info(
                f"Thinker initialized with {self.current_thinker_model}.")
    ...
    def send_prompt(self, prompt: str, image_path: str = None, yolo: bool = False, audio_path: str = None):
        ...
        if not self.running or not self.thinker_chat:
            logging.error(
                f"Attempted to send prompt but session is not running. running: {self.running}, thinker_chat: {self.thinker_chat is not None}")
                    ...
                    for file_path in response['media']:
                        if is_low_token:
                            ...
                            try:
                                ...
                                logging.error(
                                    f"Failed to attach multimodal file {file_path}: {e}")
    ...
    def _process_thought(self, prompt, image_path, yolo, audio_path=None):
        ...
        if image_path:
            if is_low_token:
                content.append(
                    "[Image attachment skipped: Low Token Mode is active]")
            else:
                try:
                    import mimetypes
                    mime_type, _ = mimetypes.guess_type(image_path)
                    uploaded_img = self.client.files.upload(
                        file=image_path, config={'mime_type': mime_type or 'image/jpeg'})
        ...
        if audio_path:
            if is_low_token:
                content.append(
                    "[Audio attachment skipped: Low Token Mode is active]")
            else:
                try:
                    import mimetypes
                    mime_type, _ = mimetypes.guess_type(audio_path)
                    uploaded_audio = self.client.files.upload(
                        file=audio_path, config={'mime_type': mime_type or 'audio/wav'})
        ...
        for idx in range(start_idx, len(self.thinking_models)):
            ...
            logging.debug(
                f"_process_thought attempting to use model: {model_name}")
            ...
            if model_name != self.current_thinker_model or not self.thinker_chat:
                ...
                try:
                    ...
                    self.thinker_chat = self.client.chats.create(
                        model=model_name, config=self.thinker_config)

            try:
                logging.debug(
                    f"Calling self.thinker_chat.send_message_stream with content length {len(content)}...")
                ...
                for chunk_num, chunk in enumerate(response_stream):
                    logging.debug(
                        f"Received chunk {chunk_num} from response_stream")
                    if getattr(self, '_abort_flag', False):
                        logging.debug(
                            "_abort_flag is True, returning from _process_thought")
                        self.is_processing = False
                        return
                ...
                logging.debug(
                    f"About to emit thought_received. Callback count: {len(self.thought_received._callbacks)}")
                ...
                if "401" in err_str or "403" in err_str:
                    logging.warning(
                        f"API Key Invalid (401/403) with {model_name}: {err_str}")
                    ...
                    self.error_occurred.emit(
                        "The request sent to the Gemini API was invalid (400 INVALID_ARGUMENT).")
                    self.is_processing = False
                    return
                elif "499" in err_str:
                    logging.warning(
                        f"Request Cancelled (499) with {model_name}: {err_str}")
                    self.is_processing = False
                    return
                ...
                if "429" in err_str or "quota" in err_str.lower() or ("400" in err_str and "FAILED_PRECONDITION" in err_str):
                    logging.warning(
                        f"API Rate Limit/Precondition (429/400) with {model_name}: {err_str}")
                    if is_last_model:
                    ...
                    logging.warning(
                        f"Safety Trigger (200) with {model_name}: {err_str}")
                    if is_last_model:
                        self.error_occurred.emit(
                            "A safety trigger was fired which immediately stopped the agent's thoughts.")
                        self.is_processing = False
                        return
                    continue
                elif "404" in err_str:
                    logging.warning(
                        f"Model Invalid (404) with {model_name}: {err_str}")
                    if is_last_model:
                        self.error_occurred.emit(
                            "The model string is invalid (404).")
                        self.is_processing = False
                        return
                    continue
                ...
                # Any other error
                logging.warning(
                    f"Thinker API Error with {model_name}: {err_str}")
                if is_last_model:
    ...
    def reformulate_query(self, user_prompt: str, history: list) -> str:
        ...
        for model_name in models:
            try:
                ...
                if "401" in err_str or "403" in err_str:
                    logging.warning(
                        f"Reformulator API Key Invalid (401/403) with {model_name}: {err_str}")
                    return user_prompt
                elif "500" in err_str or "503" in err_str:
                    logging.warning(
                        f"Reformulator Servers at Peak Capacity (500/503) with {model_name}: {err_str}")
                    return user_prompt
                elif "400" in err_str and "INVALID_ARGUMENT" in err_str:
                    logging.error(
                        f"Reformulator API Error (400 INVALID_ARGUMENT) with {model_name}: {err_str}", exc_info=True)
                    return user_prompt
                elif "499" in err_str:
                    logging.warning(
                        f"Reformulator Request Cancelled (499) with {model_name}: {err_str}")
                    return user_prompt

                # Recoverable errors (attempt fallback models)
                if "429" in err_str or "quota" in err_str.lower() or ("400" in err_str and "FAILED_PRECONDITION" in err_str):
                    logging.warning(
                        f"Reformulator API Rate Limit/Precondition (429/400) with {model_name}: {err_str}")
                    continue
                elif "200" in err_str or "safety" in err_str.lower():
                    logging.warning(
                        f"Reformulator Safety Trigger (200) with {model_name}: {err_str}")
                    continue
                elif "404" in err_str:
                    logging.warning(
                        f"Reformulator Model Invalid (404) with {model_name}: {err_str}")
                    continue

                # Any other error
                logging.warning(
                    f"Reformulator API Error with {model_name}: {err_str}")
                continue
