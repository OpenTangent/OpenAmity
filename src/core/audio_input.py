...
class AudioService:
    ...
    def _record_and_transcribe(self):
        ...
        frames = []
        silence_frames = 0
        max_recording_time = self.settings.get(
            "core.audio.max-recording-time", 60)
        silence_timeout = self.settings.get("core.audio.silence-timeout", 3.0)
        max_chunks = int(max_recording_time * SAMPLE_RATE / CHUNK_SIZE)
        silence_threshold_chunks = int(
            silence_timeout * SAMPLE_RATE / CHUNK_SIZE)
            ...
            if is_low_token or is_agy_mode:
                ...
                for model_name in models:
                    try:
                        ...
                        logging.warning(
                            f"Transcription failed with {model_name}: {e}")
                        continue

                if not text:
                    raise Exception(
                        "All Gemini models failed to transcribe audio.")
