...
"""
        ...
        self.running = True
        logging.info(
            f"AgyWorker session started using model {self.current_model}.")
    ...
    def _process_thought(self):
            ...
            while retries > 0 and not self._abort_flag:
                try:
                    ...
                    if result.returncode != 0:
                        logging.error(
                            f"agy CLI failed with {self.current_model}: {result.stderr}")
                    ...
                    try:
                        ...
                        logging.warning(
                            f"JSON Parse Error from agy CLI: {e}. Output was: {out}")
                        self.history.append(
                            {"role": "System", "content": f"CRITICAL ERROR: Failed to parse your response as JSON. Error: {e}. You MUST output pure JSON matching the schema."})
                        retries -= 1

                except Exception as e:
                    logging.error(
                        f"AgyWorker execution error with {self.current_model}: {e}", exc_info=True)
                    break  # Break retry loop to try next model
        ...
        if not self._abort_flag:
            self.error_occurred.emit(
                "The Antigravity Worker failed to initialise")
