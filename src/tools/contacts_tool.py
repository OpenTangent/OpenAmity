    ...
    def execute(self, command: str, *args, **kwargs) -> str:
        if command == "add_contact":
            ...
            success, msg = self.manager.add_contact(
                phone_number, name, relationship)
            return msg
            ...
            success, msg = self.manager.update_contact(
                phone_number, name=name, relationship=relationship)
            return msg
            ...
            if name:
                ...
                if results:
                    output = [
                        f"- {c['name']} ({c['phone_number']}) - Rel: {c.get('relationship', 'None')}" for c in results]
            ...
            output = [
                f"- {c['name']} ({c['phone_number']}) - Rel: {c.get('relationship', 'None')}" for c in contacts]
            return "Address Book:\n" + "\n".join(output)
